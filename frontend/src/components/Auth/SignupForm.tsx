...
import { Mail, Lock, User, Eye, EyeOff, TrendingUp, CheckCircle } from 'lucide-react';
...
const SignupForm: React.FC<SignupFormProps> = ({ onToggleMode }) => {
  ...
  const { signup, sendEmailOtp, verifyEmailOtp } = useAuth();
  const { addNotification } = require('../../context/NotificationContext').useNotifications ? require('../../context/NotificationContext').useNotifications() : { addNotification: () => {} };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        const token = await sendEmailOtp(formData.email);
        setOtpToken(token);
      }
    } catch (error) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (!otpToken) return;
    setIsLoading(true);
    const ok = await verifyEmailOtp(formData.email, otp, otpToken);
    setIsLoading(false);
    if (ok) {
      setOtpVerified(true);
      addNotification({ title: 'Email verified', message: 'Your account is ready.', type: 'success' });
      navigate('/dashboard');
    } else {
      ...
      <div className="w-full max-w-md">
        <div className="bg-gray-800/60 backdrop-blur-xl border border-gray-700/50 rounded-2xl shadow-2xl p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-white mb-2">Join CryptoArmory</h1>
          ...
          {otpToken && (
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              <div className="rounded-xl border border-blue-500/30 bg-blue-900/20 p-4 text-blue-200 text-sm">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-200 disabled:opacity-50"
              >
